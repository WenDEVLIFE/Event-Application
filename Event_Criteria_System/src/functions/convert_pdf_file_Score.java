 ...
 */
public class convert_pdf_file_Score {
    private final String eventname;

    public convert_pdf_file_Score(String eventname) {
        this.eventname = eventname;
    }

    public void pdf() throws IOException {
        DefaultTableModel model2 = dashboard.model1;
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
               ...
               float schoolNameX = 220; // Adjust this value based on your desired position
                ...
                contentStream.setFont(font, 12);
                contentStream.showText("Score Table");
                contentStream.endText();

                int totalScore = (int) calculateTotalScore(model2);
                contentStream.beginText();
...
for (int row = 0; row < model2.getRowCount(); row++) {
 ...
 private double calculateTotalScore(DefaultTableModel model) {
    double totalScore = 0.0;
    int scoreColumnIndex = 3; // Assuming the score is in the fourth column, adjust if needed

    for (int row = 0; row < model.getRowCount(); row++) {
        totalScore += (double) model.getValueAt(row, scoreColumnIndex);
    }
  ...
  private float[] calculateColumnWidths(DefaultTableModel model, PDType0Font font) throws IOException {
    ...
    int pdfCol = 0; // Declare pdfCol outside the loop
    ...
    for (int col = 0; col < numColumns; col++) {
        ...
        columnWidths[pdfCol++] = maxWidth + 10; // Add padding
    }

    return Arrays.copyOf(columnWidths, pdfCol); // Trim the array to exclude the "Delete" column
}
